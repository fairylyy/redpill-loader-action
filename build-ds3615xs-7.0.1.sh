...
# build redpill-load
cd redpill-load
sed -i -e 's\0x0001\0x0002\g' -e 's\0x46f4\0x0002\g' -e 's\1234XXX123\1230LWN003637\g' -e 's\MAC1ADDRESS\245EBE0299E8\g' -e 's\MAC2ADDRESS\001B21BC18BB\g' ${root}/user_config.DS3615xs.json
cp -f ${root}/user_config.DS3615xs.json ./user_config.json
sudo ./ext-manager.sh add 'https://github.com/jumkey/redpill-load/raw/develop/redpill-virtio/rpext-index.json'
sudo ./ext-manager.sh add 'https://raw.githubusercontent.com/pocopico/rp-ext/master/mpt2sas/rpext-index.json'
